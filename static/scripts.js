...
async function getData() {
    ...
    if(currentPage!= null){
        isLoading = true;
        if (currentKeyword) {
            url = `https://tdt-tw.online/api/attractions?page=${currentPage}&keyword=${encodeURIComponent(currentKeyword)}`;
        } else {
            url = `https://tdt-tw.online/api/attractions?page=${currentPage}`;
        }
...
async function addMrtsList() {
    let response = await fetch("https://tdt-tw.online/api/mrts", {
        method: "GET",
        headers: {
            "Content-Type": "application/json"
        }
...
async function getTopList() {
    try {
        let response = await fetch("https://tdt-tw.online/api/top-attractions", {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
...
async function fetchAttractionsByKeyword(keyword) {
    ...
    let response = await fetch(`https://tdt-tw.online/api/attractions?page=0&keyword=${encodeURIComponent(keyword)}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json"
        }
